...
(define-constant ERR_INVALID_INPUT (err u111)) ;; New error constant for general input validation
...
(define-public (create-timber-parcel
    ...
    (let ((parcel-id (+ (var-get total-parcels) u1))
        ...
        ;; Assertions for string inputs
        (asserts! (> (len certification) u0) ERR_INVALID_INPUT)
        (if (is-some metadata-uri)
            (asserts! (> (len (unwrap-panic metadata-uri)) u0) ERR_INVALID_INPUT)
            true
        )
...
(define-public (verify-parcel (parcel-id uint))
    (let ((parcel-data (unwrap! (map-get? timber-parcels parcel-id) ERR_NOT_FOUND)))
        ;; Assertions for parcel-id
        (asserts! (> parcel-id u0) ERR_INVALID_INPUT)
...
(define-public (submit-harvest-report
    ...
    (let ((parcel-data (unwrap! (map-get? timber-parcels parcel-id) ERR_NOT_FOUND))
        ...
        ;; Assertion for report-uri
        (asserts! (> (len report-uri) u0) ERR_INVALID_INPUT)
...
(define-public (add-authorized-verifier (verifier principal))
    (begin
        ...
        ;; Assertion for verifier principal
...
(define-public (update-parcel-metadata (parcel-id uint) (new-metadata-uri (string-ascii 256)))
    (let ((parcel-data (unwrap! (map-get? timber-parcels parcel-id) ERR_NOT_FOUND)))
        ...
        ;; Assertion for new-metadata-uri
        (asserts! (> (len new-metadata-uri) u0) ERR_INVALID_INPUT)
